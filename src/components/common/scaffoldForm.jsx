...
function ScaffoldForm() {
  ...
  async function handleGeneration() {
    ...
    try {
      ...
      const response = await invoke('generate_template', {
    ...
    >
      ...
      <div className="mb-11">
        <h1 className="text-center text-2xl text-muted">
          Rename, compress and generate the best practice Automation Anywhere
          template
        </h1>
        <p className="text-center text-xs italic text-muted">
          This application is made in order to automate the generation of best
          practice template, which return a <Tag text={'.zip'} /> file that you
          only have to import in your Control Room
        </p>
      </div>
      ...
      <AddPhase
        onAdd={(phaseName) => {
          ...
          if (lowerNames.includes(name.toLowerCase())) {
            toast.warning(
              `You cannot set the phase '${phaseName}' because already exist in phases added`,
            );
            return;
          }
          setScaffoldData((prev) => ({
            ...prev,
            phases: [...prev.phases, phaseName],
          }));
        }}
      />
